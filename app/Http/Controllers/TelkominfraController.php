  ...
  public function show(string $id) 
    {
        ...
        foreach ($perjalananDatas as $dataItem) {
            ...
            try {
                ...
                // Asumsi $this->parseNmfSinyal ada dan dapat dipanggil
                ...
                foreach ($dataSinyal as $sinyal) {
                    ...
                    if ($rawTimestamp) {
                        try {
                            // Pastikan Carbon sudah di-import: use Carbon\Carbon;
        ...
        // --- START: LOGIKA FALLBACK UNTUK DATA YANG HILANG ---
        
        // Cek apakah data 'Before' benar-benar kosong (semua count 0)
        $isBeforeEmpty = $avgAccumulators['Before']['rsrp_count'] === 0 &&
                         $avgAccumulators['Before']['rsrq_count'] === 0 &&
                         $avgAccumulators['Before']['sinr_count'] === 0;

        // Cek apakah data 'After' benar-benar kosong (semua count 0)
        $isAfterEmpty = $avgAccumulators['After']['rsrp_count'] === 0 &&
                        $avgAccumulators['After']['rsrq_count'] === 0 &&
                        $avgAccumulators['After']['sinr_count'] === 0;

        if ($isBeforeEmpty && !$isAfterEmpty) {
            // Skenario 1: Hanya data 'After' yang ada. Duplikasi 'After' ke 'Before'.
            // Ini akan membuat selisih di tampilan menjadi 0.
            ...
            // Skenario 2: Hanya data 'Before' yang ada. Duplikasi 'Before' ke 'After'.
            // Ini akan membuat selisih di tampilan menjadi 0.
            $signalAverages['After'] = $signalAverages['Before'];
            Log::info("Data 'After' kosong, menggunakan data 'Before' sebagai fallback.");
            
        } elseif ($isAfterEmpty && $isBeforeEmpty) {
            // Skenario 3: Keduanya kosong. Biarkan rata-rata tetap 0 (akan ditampilkan sebagai N/A di view).
     ...
     public function store(Request $request)
    {
        ...
        try {
            ...
            // C. Parse Header NMF & Waktu
            // Asumsi metode parseNmfHeader ada
            $nmfHeaderData = $this->parseNmfHeader($oldPath); 
            
            // D. <<< BARU: AMBIL WAKTU START/STOP >>>
            $nmfTimes = $this->extractNmfTimes($oldPath); 
            // Ambil perangkat dari header
            $perangkat = $nmfHeaderData['perangkat'] ?? 'Unknown Device';
            ...
            $results = DB::transaction(function () use ($validatedData, $finalIdPerjalanan, $perangkat, $tempUniqueFileName, $nmfTimes) {
                ...
                $perjalananData = PerjalananData::create([
                    ...
                    'file_nmf' => $tempUniqueFileName,
                    'status' => $validatedData['status'],
                    // <<< BARU: Tambahkan Timestamps dari NMF Header >>>
