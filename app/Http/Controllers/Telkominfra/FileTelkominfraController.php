    ...
    public function parseNmfSinyal(string $nmfPath, int $perjalananId): array
    {
        ...
        $resolveBandFrequency = function (?int $earfcn) {
            ...
            if ($earfcn !== null) {
                ...
                // --- Logic untuk Menentukan Bandwidth dan N ---
                if (is_int($band) && isset($bandwidth_mapping[$band])) {
                    $bandwidth = $bandwidth_mapping[$band];
                    // Hitung N (Resource Blocks).
                    // N_RB = Bandwidth (MHz) * 5 (untuk channel 1.4, 3, 5, 10, 15, 20 MHz)
                    $n_value = $bandwidth * 5; 
                } else {
                    // Jika Band tidak diketahui
                    $bandwidth = null;
                    $n_value = null;
                }
                // ----------------------------------------------
